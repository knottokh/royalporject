var $adminTables= {
	...
	ToCalendarDateTime:function(divid){
	 ...
	 dialogShow:function ($row, id){
			...
			$adminTables.ToCalendarDate("#sales_date");
            		...
            		var attno = $adminTables.objdata();
				 					...
				 					$pageEntity.GetData($globalKudo.apipath,objgetdata,function(data){
				 												...
				 												//alert(jsonobj[0].Name);
				 												 $adminTables.$taskDialog.find('textarea[name=sales_detail]').html(jsonobj[0].sales_detail);
				 												 $adminTables.$taskDialog.find('input[name=address_id]').val(jsonobj[0].address_id);	
				 												 $adminTables.$taskDialog.find('input[name=address_moo]').val(jsonobj[0].address_moo);
				 												 $adminTables.$taskDialog.find('#sales_date').data('DateTimePicker').date($pageEntity.MysqlDatetoDate(jsonobj[0].sales_date));	
                                                                 $adminTables.$taskDialog.find('input[name=sales_total]').val(jsonobj[0].sales_total);
                                                                 $adminTables.$taskDialog.find('input[name=net_total]').val(jsonobj[0].net_total);
   ...
   addToDatabase:function(itemid){
   		if( $adminTables.$formValid.valid()){
   					//	var file_data = $adminTables.$taskDialog.find('#files').prop('files')[0];   
					    var sales_detail	= $adminTables.$taskDialog.find('textarea[name=sales_detail]').html();   					
   						var address_id		= $adminTables.$taskDialog.find('input[name=address_id]').val();
					    var address_moo 	= $adminTables.$taskDialog.find('input[name=address_moo]').val();
					    var sales_date 	= $adminTables.getDateToDb('sales_date');
					    var sales_total 	= $adminTables.$taskDialog.find('input[name=sales_total]').val();
					    var net_total 	= $adminTables.$taskDialog.find('input[name=net_total]').val();
					    var remark 	= $adminTables.$taskDialog.find('textarea[name=remark]').html();
   					 	var attno = $adminTables.objdata();
	 								attno.sales_detail = sales_detail;
	 								attno.address_id = address_id;
									attno.address_moo = address_moo;	
									attno.sales_date = sales_date;
	 								attno.sales_total = sales_total;
	 								attno.net_total = net_total;
									attno.remark = remark;	
	 						if(itemid){
	 										attno.value = itemid;
	 								}	
