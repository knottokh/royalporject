            		...
            		var attno = $adminTables.objdata();
				 					...
				 					$pageEntity.GetData($globalKudo.apipath,objgetdata,function(data){
				 												...
				 												//alert(jsonobj[0].Name);
				 												 $adminTables.$taskDialog.find('input[name=fund_name]').val(jsonobj[0].fund_name);
				 												 $adminTables.$taskDialog.find('textarea[name=history]').html(jsonobj[0].history);
   ...
   addToDatabase:function(itemid){
   		if( $adminTables.$formValid.valid()){
   					//	var file_data = $adminTables.$taskDialog.find('#files').prop('files')[0];   
					    var fund_name 	= $adminTables.$taskDialog.find('input[name=fund_name]').val();
					    var history 	= $adminTables.$taskDialog.find('textarea[name=history]').html();
   					 	var attno = $adminTables.objdata();
	 								attno.fund_name = fund_name;
	 								attno.history = history;
