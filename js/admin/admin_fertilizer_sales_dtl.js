            		...
            		var attno = $adminTables.objdata();
				 					...
				 					$pageEntity.GetData($globalKudo.apipath,objgetdata,function(data){
				 												...
				 												//alert(jsonobj[0].Name);
				 												 $adminTables.$taskDialog.find('input[name=sales_id]').val(jsonobj[0].sales_id);	
				 												 $adminTables.$taskDialog.find('input[name=fertilizer_id]').val(jsonobj[0].fertilizer_id);
				 												 $adminTables.$taskDialog.find('input[name=sales_total]').val(jsonobj[0].sales_total);	
				 												 $adminTables.$taskDialog.find('input[name=cost_unit]').val(jsonobj[0].cost_unit);
   ...
   addToDatabase:function(itemid){
   		if( $adminTables.$formValid.valid()){
   					//	var file_data = $adminTables.$taskDialog.find('#files').prop('files')[0];   
   						var sales_id		= $adminTables.$taskDialog.find('input[name=sales_id]').val();
					    var fertilizer_id 	= $adminTables.$taskDialog.find('input[name=fertilizer_id]').val();
					    var sales_total 	= $adminTables.$taskDialog.find('input[name=sales_total]').val();
					    var cost_unit 	= $adminTables.$taskDialog.find('input[name=cost_unit]').val();
					    var remark 	= $adminTables.$taskDialog.find('textarea[name=remark]').html();
   					 	var attno = $adminTables.objdata();
	 								attno.sales_id = sales_id;
	 								attno.fertilizer_id = fertilizer_id;
									attno.sales_total = sales_total;	
	 								attno.cost_unit = cost_unit;
									attno.remark = remark;	
	 						if(itemid){
	 										attno.value = itemid;
	 								}	
