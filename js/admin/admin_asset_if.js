            		...
            		var attno = $adminTables.objdata();
				 					...
				 					$pageEntity.GetData($globalKudo.apipath,objgetdata,function(data){
				 												...
				 												//alert(jsonobj[0].Name);
				 												 ...
				 												 $adminTables.$taskDialog.find('textarea[name=asset_detail]').html(jsonobj[0].asset_detail);	
				 												 $adminTables.$taskDialog.find('input[name=asset_amt]').val(jsonobj[0].asset_amt);	
				 												 $adminTables.$taskDialog.find('input[name=asset_unit]').val(jsonobj[0].asset_unit);
				 												 $adminTables.$taskDialog.find('input[name=asset_unit_cost]').val(jsonobj[0].asset_unit_cost);
				 												 $adminTables.$taskDialog.find('input[name=acctype_id]').val(jsonobj[0].acctype_id);
				 												 $adminTables.$taskDialog.find('input[name=acct_flag]').val(jsonobj[0].acct_flag);
   ...
   addToDatabase:function(itemid){
   		if( $adminTables.$formValid.valid()){
   					//	var file_data = $adminTables.$taskDialog.find('#files').prop('files')[0];   
   					    ...
   					    var asset_detail	= $adminTables.$taskDialog.find('textarea[name=asset_detail]').html();
   						var asset_amt		= $adminTables.$taskDialog.find('input[name=asset_amt]').val();
					    var asset_unit 	= $adminTables.$taskDialog.find('input[name=asset_unit]').val();
					    var asset_unit_cost 	= $adminTables.$taskDialog.find('input[name=asset_unit_cost]').val();
					    ...
					    var acct_flag 	= $adminTables.$taskDialog.find('input[name=acct_flag]').val();
   					 	var attno = $adminTables.objdata();
	 								...
	 								attno.asset_detail = asset_detail;
									attno.asset_amt = asset_amt;	
									attno.asset_unit = asset_unit;
	 								attno.asset_unit_cost = asset_unit_cost;
									attno.acctype_id = acctype_id;	
									attno.acct_flag = acct_flag;	
	 						if(itemid){
	 										attno.value = itemid;
	 								}	
